...
export default class Graph {
  ...
  private readonly xMoveRange: number;
  private readonly outerMoveRange: number;
  private readonly colorFrom: p5.Color;
  private readonly colorTo: p5.Color;
  ...
  public draw(): void {
    ...
    this.p.noFill();
    const cp = Graph.arrayComplate(this.centerPoints);
    const op = Graph.arrayComplate(this.outerPoints);
    const co = Graph.arrayComplate(this.centerOffset);
    const oo = Graph.arrayComplate(this.outerOffset);
    this.p.beginShape();
    ...
    this.lerpedArray.forEach((points, index): void => {
      ...
      this.p.fill(c);
      const pp = Graph.arrayComplate(points);
      this.drawShape(pp);
    });
  ...
  private static arrayComplate(arr: p5.Vector[]): p5.Vector[] {
    const newarr = [...arr];
